...
public class Square
{
	...
	public class Permutation
	{
		...
		public IEnumerable<SquareMatrix<int>> GetOrientations()
		{
			var values = Matrix;
			yield return values;
			var mirror = SquareMatrix<int>.Create(values.GetMirrorX());
			yield return mirror;
			for (var i = 0; i < 3; i++)
			{
				values = SquareMatrix<int>.Create(values.GetRotatedCW());
				yield return values;
				mirror = SquareMatrix<int>.Create(mirror.GetRotatedCW());
				yield return mirror;
			}
		}
	...
	readonly ConcurrentDictionary<string, Lazy<Permutation>> _registry = new();

	public Permutation GetPermutation(SquareMatrix<int> values)
		=> _registry.GetOrAdd(values.ToMatrixString(),
